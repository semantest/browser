...
class WebBuddyExtension {
    ...
    initializeLearningSystem() {
        ...
        this.registerHandler('userGuidanceRequested', {
            ...
            canHandle: (eventType) => eventType === 'userGuidanceRequested'
        });
        this.registerHandler('automationRequested', {
            handle: this.handleAutomationRequest.bind(this),
            canHandle: (eventType) => eventType === 'automationRequested'
        });
    }
    /**
     ...
     */
    async handleUserGuidanceRequest(event) {
        const guidanceEvent = event;
        if (!this.learningUI) {
     ...
     */
    async startAutomationRecording(guidanceEvent) {
     ...
     */
    setupRecordingControls(guidanceEvent) {
     ...
     */
    async stopAutomationRecording(guidanceEvent) {
        if (!this.playwrightRecorder || !this.learningUI) {
            return;
        }
        // Stop recording
        ...
        if (reviewResult.approved) {
            ...
            }, guidanceEvent.correlationId, guidanceEvent.website, guidanceEvent.tabId);
     ...
     */
    async executeStoredAutomation(automation, event) {
        if (!this.automationManager) {
            throw new Error('Automation manager not initialized');
        }
        const result = await this.automationManager.executeAutomation(automation.id, event.payload.parameters, event.payload.context);
        return {
            status: 'executed',
            automation: {
                id: automation.id,
     ...
     */
    async handleReusePrompt(automation, event) {
        ...
        if (userChoice.doNotAskFor) {
            this.automationManager.setUserPreference(event.type, event.payload.action, event.website || 'any', userChoice);
        }
        // Handle user's choice
 ...
 */
class BrowserLearningUI {
    currentDialog;
    recordingIndicator;
    async showReuseDialog(automation, event) {
    ...
    createGuidanceDialog(event, onResponse) {
    ...
    `;
        // Add event handler
        window.handleGuidanceChoice = (optionId) => {
            onResponse({
                action: optionId,
    ...
    createScriptReviewDialog(script, onResponse) {
        const dialog = document.createElement('div');
        dialog.innerHTML = `
      <div style="
        position: fixed;
    ...
    `;
        // Add event handler
        window.handleScriptReview = (approved) => {
            const textarea = dialog.querySelector('#script-editor');
            onResponse({
                approved,
    ...
    createReuseDialog(automation, event, onResponse) {
        const dialog = document.createElement('div');
    ...
    `;
        // Add event handler
        window.handleReuseChoice = (action) => {
            const dontAskCheckbox = dialog.querySelector('#dont-ask-checkbox');
            const durationSelect = dialog.querySelector('#dont-ask-duration');
            let doNotAskFor = undefined;
 ...
 */
class BrowserPlaywrightRecorder {
    recording = false;
    actions = [];
    startTime = 0;
    context;
...
class ContentScriptIntegration {
     ...
     */
    async handleContentScriptMessage(message) {
